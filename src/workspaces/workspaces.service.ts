...
export class WorkspacesService {
  ...
  async create(auth: AuthContext, name: string) {
    ...
    await this.db.withTransaction(async (client) => {
      ...
      await client.query(
        ...
        [membershipId, workspaceId, auth.userId, 'owner'],
      );
      if (auth.userId) {
      ...
      await client.query(
        ...
        [randomUUID(), workspaceId, auth.type, auth.actorId, 'workspace.created', JSON.stringify({ name })],
      );
    });
  ...
  async addMember(auth: AuthContext, workspaceId: string, userId: string, applyDefaults = true) {
    await this.permissions.assertPermission(auth, workspaceId, 'members.manage');
    const membershipId = randomUUID();
    await this.db.query(
      'INSERT INTO workspace_memberships (id, workspace_id, user_id, role) VALUES ($1, $2, $3, $4)',
      [membershipId, workspaceId, userId, 'member'],
    );
    if (applyDefaults) {
      for (const permission of ['forms.manage', 'submissions.read']) {
        await this.db.query(
  ...
  async updateMemberPermissions(auth: AuthContext, workspaceId: string, memberId: string, assignments: Array<Record<string, string | null>>) {
    ...
    for (const assignment of assignments) {
      await this.db.query(
        ...
        [randomUUID(), workspaceId, memberId, member.rows[0].user_id, assignment.permission_id, assignment.effect, assignment.scope_type, assignment.scope_id],
      );
    }
    return { updated: true };
  }
