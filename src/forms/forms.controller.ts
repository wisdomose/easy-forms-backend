...
export class FormsController {
  ...
  @Get('forms/:id/webhooks/:webhookId/deliveries')
  async listWebhookDeliveries(@Req() req: RequestWithAuth, @Param('id') id: string, @Param('webhookId') webhookId: string) {
    return { data: await this.service.listWebhookDeliveries(req.auth!, id, webhookId) };
  }
  ...
  async presign(@Req() req: RequestWithAuth, @Param('id') id: string, @Body() body: { field_key: string; filename: string; mime: string; size_bytes?: number }) {
    return { data: await this.service.createPresignedUpload(req.auth!, id, body.field_key, body.filename, body.mime, body.size_bytes) };
  }

  @Get('forms/:id/files')
  async listFiles(@Req() req: RequestWithAuth, @Param('id') id: string) {
    return { data: await this.service.listFiles(req.auth!, id) };
  }
